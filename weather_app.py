...
- The weather data is from http://caiyunapp.com
...
API_TOKEN = os.getenv("CY_TOKEN", "TAkhjf8d1nlSlspN")

response_current = requests.get(
    f"http://api.caiyunapp.com/v2.6/{API_TOKEN}/{lng},{lat}/weather?granu=realtime,minutely&lang=en_US"
)
...
current = result_current["result"]["realtime"]
...
st.info(
    ...
    f"The wind is coming from {common_dir}."
    "\n\n\n"
    f"{result_current['result']['forecast_keypoint']}"
)

st.subheader("Week ahead")

st.write(
    "Temperature and rain forecast one week ahead & city location on the map",
    unsafe_allow_html=True,
)

with st.spinner("Loading..."):
    response_hourly = requests.get(
        f"http://api.caiyunapp.com/v2.6/{API_TOKEN}/{lng},{lat}/hourly?fields=tmp,precipitation&hourlysteps=72"
    )
    response_hourly.raise_for_status()
    response_hourly_data = json.loads(response_hourly._content)
    result_hourly = response_hourly_data["result"]["hourly"]
    ...
    for i in range(len(result_hourly["temperature"])):
        hourly["time"].append(result_hourly["temperature"][i]["datetime"])
        hourly["temperature_2m"].append(result_hourly["temperature"][i]["value"])
        hourly["precipitation"].append(result_hourly["precipitation"][i]["value"])
    ...
    timezone_str = response_hourly_data["timezone"]
    ...
    week_ahead = pd.to_datetime(hourly_df["Week ahead"], format="ISO8601")

    # Add traces
    fig.add_trace(
        go.Scatter(
            x=week_ahead + tzoffset,
    ...
    fig.add_annotation(
        x=time_now,
        y=max(hourly_df["Temperature °C"]) + 5,
    ...
    fig.update_yaxes(
        range=[
            min(hourly_df["Temperature °C"]) - 10,
    ...
    fig.update_yaxes(
        range=[
            min(hourly_df["Precipitation mm"]) - 2,
    ...
    folium.Marker(
        [lat, lng], popup=city + ", " + country, tooltip=city + ", " + country
    ).add_to(m)
    ...
    # Concluding remarks
    st.write(
        "Weather data source: [http://caiyunapp.com](http://caiyunapp.com) \n\n"
